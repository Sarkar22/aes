// Pipelined AES encryption core (FIPS 197 §5.1)
...
// Pipeline state registers: pipe[0] after initial AddRoundKey, pipe[r] after round r
reg [127:0] pipe [0:Nr-1];
reg         vpipe [0:Nr-1];

// Stage 0: initial AddRoundKey with rk[0]
wire [127:0] stage0_out = plaintext ^ round_keys[0*128 +: 128];
...
// Stages 1..Nr-1: full rounds with MixColumns
genvar r;
generate
    for (r = 1; r <= Nr-1; r = r+1) begin : mid_rounds
        wire [127:0] rout;
        aes_round #(.FINAL(0)) u_r (
            .state_in (pipe[r-1]),
            .round_key(round_keys[r*128 +: 128]),
            .state_out(rout)
        );
        always @(posedge clk) begin
...
// Stage Nr: final round, no MixColumns
wire [127:0] final_out;
aes_round #(.FINAL(1)) u_final (
    .state_in (pipe[Nr-1]),
    .round_key(round_keys[Nr*128 +: 128]),
    .state_out(final_out)
);

always @(posedge clk) begin
    if (rst) begin ciphertext <= 128'b0; valid_out <= 1'b0; end
    else     begin ciphertext <= final_out; valid_out <= vpipe[Nr-1]; end
end
